...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    n_1, n_2 = x_1.size(0), x_2.size(0)
    norms_1 = torch.sum(x_1**2, dim=-1, keepdim=True)
    norms_2 = torch.sum(x_2**2, dim=-1, keepdim=True)
    norms = (norms_1.expand(n_1, n_2) + norms_2.transpose(0, 1).expand(n_1, n_2))
    distances_squared = norms - 2 * torch.matmul(x_1, x_2.transpose(1,0))
    distances = torch.exp(-distances_squared / sigma)
...
def hinge_loss(scores, labels):
    ...
    return torch.mean(torch.max(torch.zeros(scores.size(0), labels.size(0)), 1 - scores * labels))
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def linear(x_1, x_2):
        ...
        '''
        return torch.matmul(x_1, x_2.transpose(1,0))
